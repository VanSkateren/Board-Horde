...
namespace Adrenak.Tork {
	public class Wheel : MonoBehaviour {
		...
		private float m_PrevCompressionDist;
		...
		private const float k_RayStartHeight = 1;
		...
		private bool WheelRaycast(float maxDistance, ref RaycastHit nearestHit) {
			RaycastHit hit;
		...
		private void CalculateFriction() {
			...
			// Apply brake force
			float brakeForceMag = brakeTorque / radius;
	...
	private void OnDrawGizmos() {
			...
			Vector3 p1 = transform.position + transform.up * k_RayStartHeight;
			Vector3 p2 = transform.position - transform.up * (GetRayLen() - k_RayStartHeight);
			Handles.DrawLine(p1, p2);

			Vector3 pos = transform.position + (-transform.up * (GetRayLen() - k_RayStartHeight - CompressionDistance - radius));
